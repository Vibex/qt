/*
Title: Font Library v0.3 (Preview)
...
;   error and return without showing the Font dialog box.
;
...
Fnt_ChooseFont(hOwner=0,ByRef r_Name="",ByRef r_Options="",p_Effects=True,p_Flags=0)
    ...
    CFSize:=4+(PtrSize*3)+12+(PtrSize*5)+12
    VarSetCapacity(CHOOSEFONT,CFSize,0)
    NumPut(CFSize,  CHOOSEFONT,0,"UInt")                ;-- lStructSize
    NumPut(hOwner,  CHOOSEFONT,4,PtrType)               ;-- hwndOwner
    NumPut(&LOGFONT,CHOOSEFONT,4+(PtrSize*2),PtrType)   ;-- lpLogFont
    NumPut(p_Flags, CHOOSEFONT,4+(PtrSize*3)+4,"UInt")  ;-- Flags
    NumPut(o_Color, CHOOSEFONT,4+(PtrSize*3)+8,"UInt")  ;-- rgbColors

    if o_SizeMin
        NumPut(o_SizeMin,CHOOSEFONT,4+(PtrSize*3)+12+(PtrSize*5)+4,"Int")
            ;-- nSizeMin

    if o_SizeMax
        NumPut(o_SizeMax,CHOOSEFONT,4+(PtrSize*3)+12+(PtrSize*5)+8,"Int")
            ;-- nSizeMax
    ...
    if p_Flags & CF_EFFECTS
        {
        l_Color:=NumGet(CHOOSEFONT,4+(PtrSize*3)+8,"UInt")

        ;-- Covert to RGB in Hex format
...
Fnt_GetListOfFonts(p_CharSet="",p_Name="")
    ...
    ;-- Enumerate fonts
    hDC:=DllCall("GetDC","UInt",HWND_DESKTOP)
    DllCall("EnumFontFamiliesEx" . (A_IsUnicode ? "W":"A")
        ,"UInt",hDC                                     ;-- hdc
        ,"UInt",&LOGFONT                                ;-- lpLogfont
        ,"UInt",RegisterCallback("Fnt_EnumFontFamExProc","Fast")
            ;-- lpEnumFontFamExProc
        ,"UInt",0                                       ;-- lParam
        ,"UInt",0)                                      ;-- dwFlags
    
    DllCall("ReleaseDC","UInt",HWND_DESKTOP,"UInt",hDC)
...
;   functions or by the AutoHotkey "gui Font" command. 
;
; Parameters:
;
;   hFont - Handle to a logical font. [Optional] Set to 0 (the default) to use
...
; Remarks:
;
; * Color is an option of the AutoHotkey "gui Font" command and of the
;   ChooseFont API and is included by these commands because text color is often
;   set with the font.  However, text color is a control attribute, not a font
;   attribute and so it is not (read: cannot be) collected/returned by this
;   function.  If text color is to be included as one of the options sent to the
;   AutoHotkey "gui Font" command or to the ChooseFont API, it must must be
;   collected and/or set independently.
;
...
Fnt_GetStringSize(hFont,p_String,ByRef r_Width="",ByRef r_Height="")
    ...
    RC:=DllCall("GetTextExtentPoint32" . (A_IsUnicode ? "W":"A")
        ,PtrType,hDC                                    ;-- hDC
